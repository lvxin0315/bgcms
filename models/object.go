...
type Object struct {
	...
	Demo       string `orm:"column(demo);size(255)"`
	DemoOk     string `orm:"column(demo_ok);size(255)"`
	Demottt    string `orm:"column(demottt);size(255)"`
}
...
func GetAllObjectCount(query map[string]string) (count int64, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Object))
	// query k=v
	for k, v := range query {
